...
工具集：POSPAL_MCP_PROFILE=default（默认，精简）或 advanced（含 raw 重复查询工具）。

## 两层 API 如何选型

- **pospal_sem_***（语义层，推荐）：自然语言查询，接受日期 YYYY-MM-DD、门店名 shop_names，返回 {ok, data} 结构。
- **pospal_***（原始层）：精确 CRUD、充值/采购/网单等；部分重叠查询仅在 advanced profile 暴露。
...
| 场景 | 优先工具 | 不要用 |
|------|----------|--------|
| 日营业额 / 客单数 | pospal_sem_get_store_sales_summary | — |
| 时段会员充值总额 | pospal_recharge_summary | pospal_list_customers 的 summary |
| 商品销售汇总 | pospal_sem_query_category_sales | — |
| 按名称/条码找商品 | pospal_sem_find_products | — |
| 查单品库存 | pospal_sem_check_product_stock | — |
| 逐笔销售流水 | pospal_sem_query_sales_detail | — |
| 商品 CRUD | pospal_get_product / pospal_create_product 等 | pospal_sem_* |
| 外卖授权与映射覆盖率 | pospal_waimai_shop_status | — |
| 外卖商品列表（含未映射） | pospal_waimai_list_products(mapping_status=…) | — |
